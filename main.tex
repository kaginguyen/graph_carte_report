...
\begin{itemize}
    ...
    \item MLP và ResNet: Mạng nơ-ron đa tầng (MLP) và phiên bản mở rộng của nó với các lớp chuẩn hóa (layer norm/batch norm) và kết nối tắt (skip-connections) của ResNet.
    ...
    \item S-LLM: Dựa trên ý tưởng từ TabLLM, nhóm thử nghiệm cách mã hóa từng hàng dữ liệu bằng một mô hình ngôn ngữ lớn (LLM) với mỗi hàng được biểu diễn dưới dạng một câu và mã hóa bằng mô hình intfloat/e5-small-v2 từ HuggingFace. Khác với TabLLM, đầu ra được chuyển vào mô hình XGB để học cho cả hai bài toán hồi quy và phân loại. Với các cột dữ liệu số thì hoặc nối trực tiếp như  các đặc trưng bổ sung đầu vào (CN), hoặc chuyển đổi thành chuỗi trước khi đưa vào LLM (EN).
    \item TabPFN: Một mô hình transformer được huấn luyện trước trên dữ liệu tổng hợp, giúp dự đoán cho các tập dữ liệu nhỏ chỉ với một lần suy luận. Các đặc trưng văn bản được xử lý dưới dạng phân loại và mã hóa với phương pháp mã hóa mục tiêu.
\end{itemize}
